...
public class Controller extends HttpServlet {
	...
	public static final String PAGE_TERMS = "/terms-of-use.jsp";
	public static final String PAGE_TERMS_DECLINED = "/terms-of-use-declined.jsp";
	public static final String PAGE_ATTRIBUTES = "/attributes.jsp";
	public static final String PAGE_ATTRIBUTES_DECLINED = "/attributes-declined.jsp";
	public static final String PAGE_RESET = "/reset-approvals.jsp";
		...
		try {
			...
			if (isGetParSet(request, GETPAR_TERMS_CONFIRM)) {
				...
				if (isGetParSet(request, GETPAR_TERMS_AGREE)) {
					...
					// check if user has to be redirected to the attributes
					// approval page
					...
					if (attributesReleased.isEmpty()) {
						LOG.info("No attributes are released for this relying party");
						response.sendRedirect(response.encodeRedirectURL(returnURL));
						return;
					}
			...
			if (isGetParSet(request, GETPAR_ATTRIBUTES_CONFIRM)) {
				LOG.info(
						"user gave attribute release consent, store, redirect to returnURL {}",
	...
	private void storeUser(UserLogInfo userInfo, String principal, String termsVersion, String entityId, String attributesReleased, boolean globalConsent) throws UApproveException {

		LOG.debug("storeUser");
		LogInfo storage = LogInfo.getInstance();
		if (userInfo == null) {
			LOG.debug("create new user");
			userInfo = storage.addUserLogInfoData(principal, "1.0",
					new Date().toString(), termsVersion, "no", entityId,
					attributesReleased);
		} else {
	...
	private void storeUserBasic(UserLogInfo userInfo, String principal,
		...
		if (userInfo == null) {
			LOG.debug("create new user");
			userInfo = storage.addUserLogInfoData(principal, "1.0",
					new Date().toString(), termsVersion, "no", null, null);
		} else {
			userInfo.setTermsVersion(termsVersion);
			userInfo.setOndate(new Date().toString());
		}
	...
	public static boolean isGetParSet(HttpServletRequest request, String par) {
	...
	public static Locale createLocale(Locale requestLocale, String enforced) {
		...
		if (enforced != null && !enforced.equals(""))
			return new Locale(enforced.substring(0, enforced.indexOf("_")), enforced.substring(enforced.indexOf("_") + 1));
	...
	public static String getResourceHost(String entityId) {
		...
		// return just the sp.example.org component out of
		...
		// return just the sp.example.org component out of
		// urn:mace:federation.org:sp.example.org
	...
	public static String getRelyingPartyName(RelyingParty relyingParty,
			Locale locale) {

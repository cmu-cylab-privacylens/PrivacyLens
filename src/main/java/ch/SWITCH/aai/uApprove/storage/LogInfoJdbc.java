 ...
 *
 * Purpose:
 * Subclass instantiating the LogInfo for JDBC based datastores
 * 
 * @author C.Witzig
 * 
 * 
...
public class LogInfoJdbc extends LogInfo
{
  private static Logger LOG = LoggerFactory.getLogger(LogInfoJdbc.class);
    ...
    /** Returns the index of the user username 
	in the table ArpUser
    ...
    */
    ...
    {
	// String sql = "select idxArpUser as idxUser from ArpUser where auUserName = '?'";
    ...
    private int getProviderIndex( String sProviderName )
	throws UApproveException
    {
	// String sql = "select idxShibProvider as idxProvider from ShibProvider where spProviderName = '?'";
	...
	try { 
	    ...
	    if ( bDebug ) 
		System.out.println("LogInfoJdbc.getProviderIndex: user = " + sProviderName
				   + " " + ex.getMessage() );
    ...
    private void addProvider( String sProviderName )
	throws UApproveException
    {
    ...
    /** Retrieves the user info from the tables ArpUser, AttrReleaseApproval
	and ShibProvider. Only returns info if the user exists and
	has at least one AttrReleaseApproval.
	@param username
	@return UserLogInfo 
    */
	...
	//	String theSQL = "select idxArpUser as idxUser, date_format(araTimeStamp,'%Y-%m-%d %H:%i%s') as ArpDate, araTermsVersion as TermsOfUseManager, araAttributes as Attributes, spProviderName as ShibProvider from ArpUser, AttrReleaseApproval, ShibProvider where auUserName='?' and idxArpUser=araIdxArpUser and araIdxShibProvider = idxShibProvider order by araTimeStamp desc";
	...
	try {
	    ...
	    String sDate = null;
	    String sGlobal = "no";
	    Map mapProviderIds = Collections.synchronizedSortedMap( new TreeMap() );
		...
		if ( bDebug ) {
		    System.out.println("LogInfoJdbc.getUserArpByname1: UserLogInfo dump:");
		    userArp.dump();
		}
	    }
	}
	catch ( SQLException ex ) {
	    System.out.println("LogInfo.getUserArpInfoByName1: SQL exception " + ex.getMessage() );
	    throw new UApproveException("LogInfoJdbc.getUserArpInfoByName1: exception " + ex.getMessage() );
	}
    ...
    /** Retrieves the user information only from the ArpUser table.
	It is possible to have a user in the ArpUser table without
	a entry in AttrReleaseApproval, namely if the user accepted
	the terms the first time but subsequently declines every 
	ArpRelease.
	@param username
	@return UserLogInfo with an empty map for the provider 
	accesses
    */
	...
	// String theSQL = "select idxArpUser as idxUser, auLastTermsVersion as TermsOfUseManager from ArpUser where auUserName='?'";
	...
	try {
		...
		userArp = new UserLogInfo( theUserName, "dummy", sDate, sTermsVersion,
					      sGlobal, mapProviderIds );
		if ( bDebug ) {
		    System.out.println("LogInfoJdbc.getUserArpByname2: UserLogInfo dump:");
		    userArp.dump();
		}
	    }
	}
	catch ( SQLException ex ) {
	    System.out.println("LogInfo.getUserArpInfoByName2: SQL exception " + ex.getMessage() );
	    throw new UApproveException("LogInfoJdbc.getUserArpInfoByName2: exception " + ex.getMessage() );
	}
    ...
    private void createUser( UserLogInfo theUserData )
	throws UApproveException
    {
	//	String sql = "insert into ArpUser (auUserName, auLastTermsVersion, auFirstAccess, auLastAccess ) values ( '?', '?', now(), now() )";
    ...
    /** Updates the relevant fields in the ArpUser table.
	The current TermsVersion is stored and the time of acess.
    */
    private void updateUser( UserLogInfo theUserData )
	throws UApproveException
    {
	//	String sql = "update ArpUser set auLastTermsVersion = '?', auFirstAccess=auFirstAccess, auLastAccess=now() where auUsername = '?'";
    ...
    private boolean hasUserGlobalArp( String theUsername )
	throws UApproveException
    {
	// String sql = "select count(*) as cnt from AttrReleaseApproval, ArpUser, ShibProvider where idxArpUser=araIdxArpUser and idxShibProvider = araIdxShibProvider and spProviderName is null and auUserName = '?'";
	...
	if ( bDebug )
	    System.out.println("LogInfoJdbc.hasUserGlobalArp: user = " + 
			       theUsername + " has global " + nCount );
	...
	@param username 
	@param idxUser: database entry ArpUser.idxArpUser
	@param bValue: if true set global arp, if false delete global arp
    */
    private void setUserArpGlobal( UserLogInfo theUserData, int idxUser, boolean bValue )
	throws UApproveException
    {
	try {
	    ...
	    if ( bValue == true ) {
		// sql = "insert into AttrReleaseApproval ( araIdxArpUser, araIdxShibProvider, araTermsVersion,araAttributes ) values ( ?, ?, '?', null )";
		...
		// sql = "delete from AttrReleaseApproval where araIdxArpUser = ? and araIdxShibProvider = ?";
    ...
    private void updateAttrReleaseApp( String theUsername, String theTermsVersion,
				       String theProviderId, String theAttr )
	throws UApproveException
    {
	try {
	    ...
	    // String theSql = "update AttrReleaseApproval set araTermsVersion = '?', araAttributes = '?' where araIdxArpUser = ? and araIdxShibProvider = ?";
	    ...
	    if ( bDebug ) 
		System.out.println("LogInfoJdbc.updateAttrReleaseApp: updating release approval for " +
				   theUsername + " " + theProviderId + " " +
				   theAttr );
	}
	catch ( Exception ex ) {
	...
	try {
	    ...
	    // String theSql = "insert into AttrReleaseApproval (araIdxArpUser, araIdxShibProvider, araTermsVersion, araAttributes ) values ( ?, ?, '?', '?' )";
	    ...
	    if ( bDebug ) 
		System.out.println("LogInfoJdbc.addAttrReleaseApp: adding release approval for " +
				   theUsername + " " + theProviderId + " " +
				   theAttr );
	}
	catch ( Exception ex ) {
	...
	try { 
	    // String sql1 = "update ProviderAccess set paIdxAttrReleaseApproval = NULL, paTimeStamp = paTimeStamp where idxArpUser = ?"
	    ...
	    // String sql2 = "delete from AttrReleaseApproval where araIdxArpUser = ?";
	    ...
	    System.out.println("LogInfoJdbc.clearUserArp: exception " + ex.getMessage() );
     ...
     */
    public static synchronized LogInfo getInstance(String filename) throws UApproveException 
    {
	if (arpLogInfo == null) {
	    arpLogInfo = new LogInfoJdbc();
	    arpLogInfo.theConfigFile = filename;
	...
	if ( ! theDB.initialize() ) 
	    throw new UApproveException("LogInfoJdbc.exception initialising the database");
	...
	if ( theSqlFile == null )
	    throw new UApproveException("LogInfoJdbc.readSqlCommands: config key not defined" );
		
	try {
	    File theFile = new File( theSqlFile );
	    if ( ! theFile.exists() || ! theFile.isFile() || ! theFile.canRead() ) {
		System.out.println("myJdbcInterface.readSqlCommands: error reading file " +
				   theSqlFile );
		throw new UApproveException("LogInfoJdbc.readSqlCommands: cannot read file " +
				       theSqlFile );
	    ...
	    throw new UApproveException("LogInfoJdbc.readSqlCommands: ex reading sql command file" +
    ...
    */
	...
	if (arpLogInfo == null)
	    throw new UApproveException("LogInfoJdbc.update: LogInfo was not yet initialized and now it need to be saved????.");
	...
	try {
	    ...
	    if ( theUserData.getGlobal().equalsIgnoreCase("yes") && bUserHasGlobalArp == false ) {
		setUserArpGlobal( theUserData, idxUser, true );
		return;
	    }

	    if ( ( theUserData.getGlobal().equalsIgnoreCase("yes") == false ) && bUserHasGlobalArp == true )
		setUserArpGlobal( theUserData, idxUser, false );

	    // if the map of providers is completely empty, delete the entries in the db
	    ...
	    Map mapDbShibProviders = theDbInfo != null ? theDbInfo.getProviderIds() : null;
	    
	    if ( mapDbShibProviders != null && mapDbShibProviders.containsKey( theProviderId ) == true )
		updateAttrReleaseApp( theUserData.getUsername(), theUserData.getTermsVersion(),
				      theProviderId, theAttr );
	    else
		addAttrReleaseApp( theUserData.getUsername(), theUserData.getTermsVersion(),
				   theProviderId, theAttr );
	}
	catch ( Exception ex ) {
	    System.out.println("LogInfoJdbc.update: exception " + ex.getMessage() );
	    LOG.error("Exception.",ex);
	    throw new UApproveException( "LogInfoJdbc.update: got exception trying to store user " + 
    ...
    public UserLogInfo addUserLogInfoData(String username,
				      String version,
				      String date,
				      String termsVersion,
				      String global, 
	...
	UserLogInfo theUserArpInfo = new UserLogInfo(username, version, date, termsVersion, global);
    ...
    public synchronized void updateArpProviderAccess( String theUsername, String theProvider, boolean bGlobal )
	throws UApproveException
    {

	if ( bDebug ) 
	    System.out.println("LogInfoJdbc.updateArpProviderAccess: cannot find approval for user "
			       + theUsername + " provider = " + theProvider + 
			       " with global access " + bGlobal );

	try {

	    // String theSQL = "update ArpUser set auFirstAccess = auFirstAccess, auLastAccess = now() where auUserName = '?'";
	    String theSQL = (String) theSqlCmds.getProperty( keySqlCmd.updUser1 );
	    theSQL = theSQL.replaceFirst( "\\?", theUsername );
	    
	    theDB.execSqlFT( theSQL, false );

	    theSQL = bGlobal == false ? 
		(String) theSqlCmds.getProperty( keySqlCmd.selIdxAttrApproval ) :
		(String) theSqlCmds.getProperty( keySqlCmd.selIdxAttrApprovalGlobal );
	    ...
	    if ( idxApproval == -1 ) 
		throw new UApproveException("LogInfoJdbc.updateArpProviderAccess: cannot find approval for user "
				       + theUsername + " provider = " + theProvider + 
				       " with global access " + bGlobal );

	    // we must get the provider from the name and not rely on the AttrReleaseApproval
	    ...
	    // theSQL = "insert into ProviderAccess ( paIdxAttrReleaseApproval, paTimeStamp ) values ( ?, ?, now() )";
	    ...
	    System.out.println("LogInfo.updateArpProviderAccess: ex = " + ex.getMessage() );
	    throw new UApproveException("LogInfoJdbc.updateArpProviderAccess: cannot find approval for user "
				   + theUsername + " provider = " + theProvider + 
				   " with global access " + bGlobal );
	}
    ...
    public synchronized void updateArpProviderAccessWithNoARA( String theUsername, String theProvider )
	throws UApproveException
    {
	...
	try {
	    ...
	    System.out.println("LogInfo.updateArpProviderAccessWithNoARA: ex = " + ex.getMessage() );
	    throw new UApproveException("LogInfoJdbc.updateArpProviderAccessWithNoARA: SQL exception " + 
				  ex.getMessage() );
	}

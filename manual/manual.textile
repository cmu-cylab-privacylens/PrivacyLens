...
In case of database connectivity problems, you may want to set the "graceful" property to "true" in @$SHIB_INSTALL$/conf/PrivacyLens.xml@:

bc. 
<property name="storage">
  <bean class="ch.SWITCH.aai.uApprove.tou.storage.JDBCStorage" init-method="initialize"
  p:dataSource-ref="PrivacyLens.dataSource" p:sqlStatements="classpath:/storage/sql-statements.properties" p:graceful="true" />
</property>
...
In case you enable the 'Terms of Use' module (disabled by default), you need to provide an appropriate text suitable for your organization.
...
In @$SHIB_INSTALL$/conf/PrivacyLens.properties@, the property @general.adminUrl@ determines the Service Provider identifier that should cause the PrivacyLens administrative interface to be presented. Define a new Service Provider with this identifier and make it known to the Identity Provider via the @$SHIB_INSTALL$/conf/relying-party.xml@ file. Provide the full set of attributes to users of this Service Provider, and require authentication.
...
* Define the metadata cache for the new virtual Service Provider (this file will be created by the Identity Provider). Modify @$SHIB_INSTALL$/conf/relying-party.xml@ and add the following:
bc.. 
<metadata:MetadataProvider id="SPAdmin" xsi:type="metadata:FileBackedHTTPMetadataProvider"
  metadataURL="https://<SP host>/admin/Shibboleth.sso/Metadata"
  backingFile="$SHIB_INSTALL$/metadata/scaleprivSPAdmin.xml">
</metadata:MetadataProvider>
p. 
* Ensure the virtual Service Provider is defined in @$SHIB_INSTALL$/conf/PrivacyLens.properties@:
bc. general.adminUrl = https://<SP host>/admin

h2. 2.5 Configure Shibboleth to honor PrivacyLens choices

Configure the Identity Provider to query PrivacyLens for attribute release choices. Edit the file @$SHIB_INSTALL$/conf/attribute-filter.xml@ and make each AttributeFilterPolicy contain a PermitValueRule that will query PrivacyLens. The language used for the script is Javascript:
...
The return value of the last function called will determine the outcome of this rule. If the return value is true, the attribute will be released by the Identity Provider.
  ...
  "SPs":[
    {
      ...
      "logo":Optional logo image file for the service provider,
      "attrGroups":[
        {

...
* This guide assumes that PrivacyLens is installed on a Linux system. It's also possible to install it on a different operating system, like Windows. In this case, you may need to adapt some paths and commands accordingly.
...
* "PrivacyLens":https://github.com/cmu-cylab-privacylens/PrivacyLens is built and its zipfile unpacked at @$PLENS_INSTALL$@ (e.g., @/usr/local/src/uApprove-#version#@).
...
* @cp@ "(file)$PLENS_INSTALL$/manual/configuration/uApprove.properties":configuration/uApprove.properties @$IDP_HOME$/conf@
* @cp@ "(file)$PLENS_INSTALL$/manual/configuration/uApprove.xml":configuration/uApprove.xml @$IDP_HOME$/conf@
...
* @cp $PLENS_INSTALL$/webapp/* $IDP_INSTALL$/src/main/webapp/uApprove@
...
** Adjust the @uApprove.dataSource@ bean in @$IDP_HOME$/conf/uApprove.xml@ (i.e., replace the @bonecp.BoneCPDataSource@ with the @c3p0.ComboPooledDataSource@, see "(file)$PLENS_INSTALL$/manual/configuration/uApprove.xml":configuration/uApprove.xml). Please refer to "JDBC Connection Pool Tuning":#JDBCConnectionPoolTuning for advanced configuration properties.
...
* @cp $PLENS_INSTALL$/lib/uApprove-#new-version#.jar $IDP_INSTALL$/lib@
...
# Copy the provided "(file)$PLENS_INSTALL$/manual/storage/sql-statements.properties":storage/sql-statements.properties to @$IDP_HOME$/conf/uApprove.sql-statements.properties@.
...
You might adjust/extend the provided resource bundles in "(file)$PLENS_INSTALL$/manual/examples/messages":examples/messages and copy them into the IdPs classpath (e.g., @$IDP_INSTALL$/src/main/webapp/WEB-INF/classes/uApprove/messages@).

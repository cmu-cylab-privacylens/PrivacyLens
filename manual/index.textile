...
bq(warn). If your upgrading from uApprove < 2.3 please read the according "upgrade information":#UpgradefromuApprove2.3 and
...
You might adjust/extend the provided resource bundles and place them in the IdPs classpath (e.g., @$IDP_INSTALL$/src/main/webapp/WEB-INF/classes/uApproveMessages@).
...
See "(file)attribute-descriptions.xml":attribute-descriptions.xml for an example how to configure the localized attribute names and descriptions.
...
bq(note). Other topics will be added upon request. Some candidates are:

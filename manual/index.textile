...
* IDP_HOME
* The Shibboleth Identity Provider is unpacked at @$IDP_INSTALL$@ (e.g., @/opt/shibboleth-identity-provider-${idp.version}@).
* The Shibboleth Identity Provider is installed at @$IDP_HOME$@ (e.g., @/opt/shibboleth-idp@).
* "uApprove":http://www.switch.ch/aai/uApprove is downloaded and unpacked at @$UAPPROVE_INSTALL$@ (e.g., @/tmp/uApprove-${uApprvove.version}@).

h2. Library Installation

* @cp $UAPPROVE_INSTALL$/lib/uApprove-${uApprvove.version}.jar $IDP_INSTALL$/lib@
...
* @cp $UAPPROVE_INSTALL$/lib/jdbc/optional/mysql-connector-java-*.jar $IDP_INSTALL$/lib@
...
bc. 
<context:property-placeholder location="$IDP_HOME$/conf/uApprove.properties" />
...
bq(warning). Verify that the IdP classpath at @$IDP_INSTALL$/lib@ doesn't contain multiple version of the same library.
